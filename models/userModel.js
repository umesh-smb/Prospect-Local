...
let userModel = {
	...
	getManagerInfo:function(user_id,office_id){
		var sql = `SELECT A.user_id,A.unique_id,A.dealer_id,
		A.email,A.user_type,A.first_name,A.last_name,A.image,A.address,A.city,A.state_id,A.phone,
		B.area_coordinates,
		B.area_id as area_id_old,(
			SELECT GROUP_CONCAT(area_id) FROM manager_regions WHERE manager_id in( SELECT user_id FROM user_master WHERE office_id = `+office_id+` )) as area_id
		FROM user_master A 
		LEFT JOIN sales_rep_regions B ON B.user_id = A.user_id
	...
	getManagerAreas:function(user_id,dealer_id,office_id , region_id,division_id,user_type){
		
		
		
		var sql =`(SELECT A.*,CONCAT(F.first_name,' ',F.last_name) as created_by,IF(LENGTH(B.residential_id) = 0,0, (LENGTH(B.residential_id) - LENGTH(REPLACE(B.residential_id, ',', '')) + 1)) as total,
		...
		LEFT JOIN user_master F ON F.user_id = A.manager_id
		LEFT JOIN residential_in_area B ON B.area_id = A.area_id
		WHERE A.status = 'Y' `;
		let cond = '';
		if(user_type == 3){
			cond+= ` AND A.manager_id IN( SELECT user_id FROM user_master WHERE dealer_id = `+dealer_id+` ) `;
		}else if(user_type == 6){
			cond+= ` AND A.manager_id IN( SELECT user_id FROM user_master WHERE division_id = `+division_id+` AND (user_type = 6 OR user_type = 7 OR user_type = 4 OR user_type = 5 ) ) `;
		}else if(user_type == 7){
			cond+= ` AND A.manager_id IN( SELECT user_id FROM user_master WHERE region_id = `+region_id+` AND ( user_type = 7 OR user_type = 4 OR user_type = 5 ) ) `;
		}else if(user_type == 4){
			cond+=`AND A.manager_id IN(SELECT user_id FROM user_master WHERE office_id = `+office_id+` AND (  user_type = 4 OR user_type = 5 ))`;
		}else{
			cond+=` AND 1=1  `;
		}
		...
		if(user_type != 3 && user_type != 2 && user_type != 1){
			var sql2 = ` UNION (SELECT A.*,CONCAT(D.first_name,' ',D.last_name) as created_by,
			IF(LENGTH(B.residential_id) = 0,0, (LENGTH(B.residential_id) - LENGTH(REPLACE(B.residential_id, ',', '')) + 1)) as total,
					(SELECT count(*) FROM customer_master C WHERE FIND_IN_SET(C.residential_id ,B.residential_id) AND C.dealer_id = `+dealer_id+` ) as home_interacted,
					ROUND(((SELECT count(*) FROM customer_master C WHERE FIND_IN_SET(C.residential_id ,B.residential_id) AND C.dealer_id = `+dealer_id+` ) / ( IF(LENGTH(B.residential_id) = 0,0, (LENGTH(B.residential_id) - LENGTH(REPLACE(B.residential_id, ',', '')) + 1))))*100) as percentage  
			FROM manager_regions A 
			LEFT JOIN user_master D ON D.user_id = A.manager_id
			LEFT JOIN residential_in_area B ON B.area_id = A.area_id
			WHERE 1=1  `;

			if(user_type == 6){
				sql2 += `AND FIND_IN_SET(A.area_id,(SELECT GROUP_CONCAT(DISTINCT  B.area_id) as aid FROM user_master A 
				LEFT JOIN sales_rep_regions B ON B.user_id = A.user_id
				WHERE A.division_id = `+division_id+` AND ( A.user_type = 5 OR A.user_type = 4 OR A.user_type = 7)  GROUP BY A.dealer_id)) `;
			}else if(user_type == 7){
				sql2 += `AND FIND_IN_SET(A.area_id,(SELECT GROUP_CONCAT(DISTINCT  B.area_id) as aid FROM user_master A 
				LEFT JOIN sales_rep_regions B ON B.user_id = A.user_id
				WHERE A.region_id = `+region_id+` AND ( A.user_type = 5 OR A.user_type = 4 )  GROUP BY A.dealer_id)) `;
			}else if(user_type == 4){
				sql2 += `AND FIND_IN_SET(A.area_id,(SELECT GROUP_CONCAT(DISTINCT  B.area_id) as aid FROM user_master A 
				LEFT JOIN sales_rep_regions B ON B.user_id = A.user_id
				WHERE A.office_id =  `+office_id+` AND  A.user_type = 5 GROUP BY A.dealer_id)) `;
			}else{
				sql2+=``;
			}
			
			sql2 += ` GROUP BY A.area_id )`;
			sql+=sql2;
		}
	...
	getManagerCoordinates:function(manager_id,office_id,region_id,division_id,user_type,dealer_id){
		//var sql = `SELECT A.* FROM manager_regions A WHERE A.status='Y' AND A.manager_id = `+manager_id;
		//var sql = `SELECT A.* FROM manager_regions A WHERE A.status='Y' AND A.manager_id in( SELECT user_id FROM user_master WHERE office_id = `+office_id+`)`;
		var sql = `(SELECT A.* FROM manager_regions A WHERE A.status='Y' `;
		let cond = '';
		if(user_type == 3){
			cond+= ` AND A.manager_id IN( SELECT user_id FROM user_master WHERE dealer_id = `+dealer_id+` ) `;
		}else if(user_type == 6){
	...
	getOwnerInfo:function(area_id){
		var sql = `SELECT B.first_name,B.last_name,B.image FROM manager_regions A
		LEFT JOIN user_master B ON B.user_id = A.manager_id
		WHERE A.area_id = `+area_id;
		return functions.selectQuery(sql);
	},
	getCoordinates:function(area_id){
		var sql = `SELECT A.* FROM manager_regions A WHERE A.area_id IN(`+area_id+`) AND A.status = 'Y'`;
		return functions.selectQuery(sql);
	},
	...
	getAssignedRepslist:function(area_id){
		...
		LEFT JOIN user_master B ON B.user_id = A.user_id
		WHERE FIND_IN_SET( `+area_id+`,A.area_id)  GROUP BY A.user_id`;
		return functions.selectQuery(sql);
	...
	getLocationDetails:function(residential_id,tableName,dealer_id){
		...
		LEFT JOIN customer_status_types D ON D.id = C.status_type
		LEFT JOIN customer_homeowner_master E ON E.latitude = A.Latitude AND E.longitude =A.Longitude AND E.dealer_id = `+dealer_id+` AND E.residential_id = `+residential_id+`
	...
	checkMultiple:function(latitude,longitude,tableName,address){
		var sql = `SELECT IF(count(*) >= 1 , 'true','false') as multiple 
		FROM `+tableName+`  
		WHERE Latitude = '`+latitude+`' AND Longitude = '`+longitude+`' AND Address != '`+address+`'` ;
		return functions.selectQuery(sql);
	},
	...
	getSaleRepLists:function(office_id,division_id,region_id,user_type,dealer_id=''){
		...
		FROM user_master A 
		LEFT JOIN sales_rep_regions B ON B.user_id = A.user_id
		WHERE `+cond+` AND A.active='Y' `;
		if(office_id != '' && office_id != undefined && office_id != null ){
			sql+= ' AND A.office_id = '+office_id;
		}
		if(user_type == 3){ 
			// Dealer 
			sql += ` AND (  A.user_type = 3 OR A.user_type = 4 OR A.user_type = 5 OR A.user_type = 6 OR A.user_type = 7 ) `;
		}else if(user_type == 6){
			// Divisional Manager
			sql += ` AND A.division_id = `+division_id+` AND ( A.user_type = 4 OR A.user_type = 5 OR A.user_type = 6 OR A.user_type = 7) `;
		}else if(user_type == 4){ 
			// office manager
			if(office_id != '' && office_id != undefined && office_id != null ){
				sql += ` AND A.office_id = `+office_id+`  AND ( A.user_type = 4 OR A.user_type = 5 ) `;
			}else{
				sql += ` AND A.division_id = `+division_id+` AND A.region_id = `+region_id+`  AND ( A.user_type = 4 OR A.user_type = 5 ) `;
			}
			...
			// Regional Manager
			sql += ` AND A.region_id = `+region_id+`  AND ( A.user_type = 4 OR A.user_type = 5 OR A.user_type = 7   ) `;
		}
		sql += ` AND A.deleted_at IS NULL  GROUP BY A.user_id ORDER BY A.first_name ASC `;
		
		
		return functions.selectQuery(sql);
	},
	getAllSaleRepLists:function(office_id,division_id,region_id,user_type,dealer_id=''){
	...
	getFormBlocks:function(fulfilment_partner_id){
		var sql = `SELECT * FROM opportunity_fields_block WHERE is_response = 'N' AND fulfilment_partner_id = `+fulfilment_partner_id+` ORDER BY display_order ASC`;
		return functions.selectQuery(sql);
	},
	getOpportunityFields:function(block_id){
		var sql = `SELECT B.type,B.mask,B.maskExpression,A.*,'' as value,'' as files FROM opportunity_fields A 
	...
	getLocationHistory:function(residential_id,latitude,longitude,tableName,dealer_id){
		...
		IF(A.home_owner_id != '',
		(CONCAT( IF(A.residential_customer_id != '' ,E.First_Name,F.first_name),' ',IF(A.residential_customer_id != '' ,E.last_name,F.Last_Name))),
		(CONCAT( IF(A.residential_customer_id != '' ,E.First_Name,D.first_name),' ',IF(A.residential_customer_id != '' ,E.last_name,D.Last_Name)))) as name,
		...
		LEFT JOIN `+tableName+` F ON F.consumer_id = A.home_owner_id 
		LEFT JOIN residential_customers E ON E.residential_customer_id = A.residential_customer_id
		WHERE A.latitude = '`+latitude+`' AND A.longitude = '`+longitude+`' AND A.dealer_id = `+dealer_id+` AND A.residential_id = `+residential_id+`
	...
	getAppointments:function(residential_id,dealer_id,location){
		
		var sql =`SELECT A.id as appointment_id,DATE_FORMAT(A.start_date,'%Y-%m-%d') as start_date,DATE_FORMAT(A.start_date,'%H:%i:%s') as start_time,
		...
		LEFT JOIN user_master B ON B.user_id = A.user_id
		WHERE A.deleted_at IS NULL AND A.residential_id = `+residential_id +` AND location = '`+location+`' AND A.dealer_id = `+dealer_id+` 
	...
	getMyAppointments:function(user_id,tableName){
		...
		DATE_FORMAT(A.start_date,'%M %d,%Y. %h:%i %p') as date,CONCAT(DATE_FORMAT(A.start_date,'%Y-%m-%d'),'T',DATE_FORMAT(A.start_date,'%H:%i:%s'),'Z') as app_date,A.user_id,A.residential_id,B.First_Name as first_name,B.Last_Name as last_name,'' as image,A.latitude,A.longitude,A.location,A.customer_name,
		A.timezone,A.selected_date,
		DATE_FORMAT(A.selected_date,'%M %d,%Y. %h:%i %p') as timezone_date,
		CONCAT(C.first_name,' ',C.last_name) as name 
		FROM customer_appointments A 
		LEFT JOIN `+tableName+` B ON B.consumer_id = A.residential_id
		LEFT JOIN user_master C ON C.user_id = A.user_id
		WHERE A.user_id = `+user_id+` ORDER BY A.start_date DESC
		`;
		//console.log(sql);
		return functions.selectQuery(sql);
	},
	getAvailableAreas:function(user_id,dealer_id){
		...
		LEFT JOIN user_master F ON F.user_id = A.manager_id
		LEFT JOIN residential_in_area B ON B.area_id = A.area_id
		WHERE A.status = 'Y' AND FIND_IN_SET ( A.area_id , ( 
	...
	getNotes:function(residential_id, dealer_id,statename){
		var sql =`SELECT id as note_id,A.user_id,A.notes,A.residential_id,DATE_FORMAT(A.created_date,'%M %d,%Y') as date,
		DATE_FORMAT(A.updated_at,'%M %d,%Y') as last_modified_date,
		CONCAT(B.first_name,' ',B.last_name) as name   
		FROM customer_notes A 
		LEFT JOIN user_master B ON B.user_id = A.user_id
		WHERE A.residential_id =`+residential_id+` AND A.dealer_id = `+dealer_id+` AND state='`+statename+`' ORDER BY A.created_date DESC`;
		return functions.selectQuery(sql);
	},
	getCustomers:function(latitude,longitude,tableName,dealer_id){
		...
		WHERE A.Latitude = '`+latitude+`' AND A.Longitude = '`+longitude+`' GROUP BY A.Address
	...
	getCustomersNew:function(latitude,longitude,tableName,dealer_id,residential_id,address,statename){
		//IF(E.id != '',(IF(E.residential_customer_id !='','false','true')),'false') as ishomeownerold,
...
(IF(A.consumer_id = F.home_owner_id,'true','false'))
) as ishomeowner,
		...
		LEFT JOIN customer_status_types D ON D.id = C.status_type
		LEFT JOIN customer_homeowner_master E ON E.residential_id = A.consumer_id AND E.dealer_id = `+dealer_id+`
		LEFT JOIN customer_homeowner_master F ON F.home_owner_id = A.consumer_id AND F.dealer_id =  `+dealer_id+`

		WHERE A.Latitude = '`+latitude+`' AND A.Longitude = '`+longitude+`' AND A.Address = '`+address+`' AND A.First_Name != ''
		...
		FROM residential_customers A
		LEFT JOIN customer_master B ON B.residential_id = A.residential_id AND B.state = '`+statename+`'
		LEFT JOIN ( SELECT T2.* FROM ( SELECT MAX(id) as id from customer_updates GROUP BY customer_id ) T1 LEFT JOIN customer_updates T2 ON T1.id = T2.id
		) C ON C.customer_id = B.customer_id 
				LEFT JOIN customer_status_types D ON D.id = C.status_type
				LEFT JOIN customer_homeowner_master E ON E.residential_customer_id = A.residential_customer_id AND E.dealer_id = `+dealer_id+`
		...
		// WHERE A.Latitude = '`+latitude+`' AND A.Longitude = '`+longitude+`' AND A.consumer_id = `+residential_id+`
	...
	getUserHistoryKDBackup:function(user_id,tableName){
		var sql =`SELECT * FROM ( 
			SELECT A.residential_id,A.latitude, A.longitude,A.first_name,A.last_name,A.email,A.phone_number,A.address,B.status_type,B.sub_status,
	...
	'' as name,
			...
			SELECT A.residential_id,A.latitude,A.longitude,
	...
	'' as name,
			...
			LEFT JOIN customer_status_types C ON C.id = B.status_type
			WHERE B.user_id = '${user_id}'  AND A.residential_id != '' 
	...
	getHomeOwnerData:function(residential_id,latitude,longitude,dealer_id){
		...
		WHERE A.latitude = '`+latitude+`' AND A.longitude = '`+longitude+`' AND A.residential_id = `+residential_id+` AND A.dealer_id = `+dealer_id;
		return functions.selectQuery(sql);
	},
	...
	checkCustomerExist:function(email,phone,first_name,last_name,dealer_id){
		...
		OR ( A.first_name = '`+first_name+`' AND A.last_name ='`+last_name+`' AND A.phone_number = '`+phone+`') ) AND A.dealer_id = `+dealer_id+` AND B.vertical_id = 1  AND B.fulfilment_partner_id = 1 AND B.active = 'Y' `;
		
		return functions.selectQuery(sql);
	},
	checkCustomerExistorNot:function(email,phone,first_name,last_name,dealer_id){
		
		var sql =`SELECT A.* FROM customer_master A 
		WHERE (( A.first_name = '`+first_name+`' AND A.last_name ='`+last_name+`' AND A.email = '`+email+`' ) 
		OR ( A.first_name = '`+first_name+`' AND A.last_name ='`+last_name+`' AND A.phone_number = '`+phone+`') ) 
		AND A.dealer_id = `+dealer_id;
		
		return functions.selectQuery(sql);
	},
	//-------------------------------
	getUserDetailsForLoginByToken:function(login_token){
		var sql = `SELECT A.user_id,A.unique_id,A.dealer_id,A.active,B.prospect_active,B.active as dealeractive,
		(SELECT count(*) FROM map_dealer_vertical WHERE map_dealer_vertical.dealer_id = A.dealer_id AND map_dealer_vertical.product_id = 2 ) as verticalscount,
		A.email,A.user_type,A.first_name,A.last_name,A.image,A.address,A.city,A.state_id,A.phone,A.password,A.otp,
		A.office_id,A.region_id,A.division_id,C.office_title as office_name
		...
		LEFT JOIN dealer_office C ON C.office_id = A.office_id
		WHERE (A.user_type = 4 OR A.user_type = 5 OR A.user_type = 6 OR A.user_type = 7 OR A.user_type = 3) 
		AND A.login_token = '`+login_token+`'`;
		
		return functions.selectQuery(sql);
	},
	getCustomersWithNoLatitude:function(){
		var sql = `SELECT * FROM customer_master  WHERE ISNULL(latitude) AND state != '' and residential_id != '' AND state = 'CO'`;
		return functions.selectQuery(sql);
	},
